...
export const HourlyForecastList: FC<HourlyForecastListProps> = ({
  hourlyData,
}) => {
  ...
  return hourlyData.map((hour, index) => (
    ...
    >
      ...
      <div className='flex items-center justify-center'>{hour.icon}</div>
      <span className='text-white text-xs md:text-sm'>{hour.temp}°</span>
    </div>
  ));
};
